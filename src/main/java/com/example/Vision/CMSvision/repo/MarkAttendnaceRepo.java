...
public interface MarkAttendnaceRepo extends JpaRepository<MarkAttendance,Integer> {
    ...
    @Query(value = "SELECT * FROM mark_attendance WHERE opened_class_id IN ?2 AND student_student_id=?1",nativeQuery = true)
    List<MarkAttendance> getMarkAttendance(int studentId,List<Integer> openClassId);
}
